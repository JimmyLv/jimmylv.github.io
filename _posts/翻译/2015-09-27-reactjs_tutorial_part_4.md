---
layout: post
title: React.js教程 第四部分：Express路由
categories: [翻译]
tags: [React, Express]
published: True
...
欢迎来到React.js/Express.js教程的第四部分。在这篇博客文章里，我们将会创建必要的Express路由来讲`json`代码提供给我们的*React*.js前端应用程序使用。我们也准备让*React*.js组件初步调用我们的服务器，以便于`json`文件可以被渲染。
...
我们想要做的第一件事情就是创建一个Express.js路由，用于一个确定的URL被点击的时候返回一个`json`。通过Express.js框架很轻松就可以达成目标。Express.js网站的安装，在[教程的第一部分](http://www.joshfinnie.com/blog/reactjs-tutorial-part-1/)已经为我们自动生成了，可以很好地用于入门。但是接下来，我们想要添加一些文件来保持组织性。
...
所以在其他事情开始之前，我们想要创建另外的文件来存放API路由。这个文件叫做`routes/api.js`就可以了，然后我们就需要稍微改变一下`app.js`来保证我们可以使用新文件。在`app.js`中，我们把`var routes = require('./routes/index');`修改成其他更有用的形式：`var index = require('./routes/index');`。然后直接在底下添加想要的新的API路由：`var api = require('./routes/api');`。最后，把这些东西添加至`app`：`app.use('/api/', api);`。我们的`app.js`就应该有部分像这样了：
...
现在我们可以添加`jobs`路由到我们`routes/api.js`，然后通过Express.js应用提供`json`了。
...
这儿有很多关于"Restful" API架构的资源，但是[Tuts+](http://code.tutsplus.com/tutorials/a-beginners-guide-to-http-and-rest--net-16340)是一个非常棒的教程，我就不想再花费太多时间去解释它了。这部分教程其实也不需要非常多的REST端点。
...
但是让我们来写第一个`GET`端点，用于列举已有的所有jobs。首先，我们想要从*React*.js代码中把`jobs`对象拿出来，然后放到`routes/api.js`里，从而我们就有一个端点可以返回`json`形式的对象。通过以下代码可以实现：
...
做到这样实际上非常简单。使用Express.js可以知道每个端点中含有哪些URL参数，我们可以添加一个像`/api/jobs/1`这样的端点，然后我们的路由就会知道我们想要`job_id == 1`的这个工作。为此，可以给`routes/api.js`添加以下代码：
...
**请注意**：这并不理想，也不具有可实践性。事实上，我们将会从数据库中查找`job_id`，而不是像我们这样很傻地使用`for -> if`语句。但是那是之后的教程了！

With these two routes, we should be able to `curl` these two endpoints and get the following results:

通过这两个路由，我们就应该可以通过`curl`来访问这两个端点，得到以下结果：
...
第一次使用*React*.js与APIs交互的时候，我推荐使用`request`包就好了。我们需要把它添加至应用程序，但是只要简单运行`npm install request --save`就好了。
...
只要我们安装好了`request`库，我们需要为`public/javascripts/scr/Jobs.jsx`文件做一些修改，从而从API中获取数据。为此，我们需要小小地修改一下`getInitialState`函数，添加`componentDidMount`函数。我们不再需要完整的初始化状态（就像我们需要的API标本那样），因为我们期望在组件在装配到应用程序的时候就从API中获取数据。我们整个`public/javascripts/scr/Jobs.jsx`文件现在长这样：
...
在上面的文件里，你可以看到我们如何使用`request`库去新建的API中获取数据，并将返回值放入`state`中。你也可以看到我们在`render`中做了修改，使我们可以使用这个API。就是这样，我们还是有一个可工作的网站，并且渲染这两个工作信息，但是现在我们的*React*.js组件是从API中获取数据，而不再是硬编码的（尽管我们的API里面还是硬编码的）。
...
通过这部分的教程，我们让应用程序使用API来获取它的数据。长路漫漫，其修远兮，但这是一个非常棒的开始。下一次我们会介绍在*React*.js里面进行路由的方式，以便于我们不仅可以看到所有的工作岗位，还可以进一步查看每个特殊的工作信息。我们还需要连接到数据库，并且从*React*中收集数据。请保持步调一致哟！
